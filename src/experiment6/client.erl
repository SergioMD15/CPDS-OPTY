...
start(ClientID, NumberEntries, Reads, Writes, Server, EntryPercentage) ->
    NumberRandomEntries = round(NumberEntries * EntryPercentage),
    Entries = lists:sublist([X || {_ , X} <- lists:sort([{rand:uniform(), E} || E <- lists:seq(1, NumberEntries)])], NumberRandomEntries),
